...
python3 train_net.py --num-gpus $num_gpus --resume --dist-url  auto \
    --config-file "configs/fsod/pretraining-${config_name}.yaml" \
    --additional-configs "$dataset_config" \
...
### Training (Two-branch)
python3 train_net.py --num-gpus $num_gpus --resume --dist-url auto \
    --config-file configs/fsod/training-${config_name}.yaml \
    --additional-configs "$dataset_config" \
        -- SOLVER.IMS_PER_BATCH $batch_size \
        SOLVER.ACCUMULATION_STEPS $accumulation_steps \
...
for i in "${few_shot_iterations[@]}"; do
    ...
    python3 train_net.py --num-gpus $num_gpus --resume --dist-url auto \
        --config-file configs/fsod/finetuning-${config_name}.yaml \
        --additional-configs "$dataset_config" \
        -- SOLVER.IMS_PER_BATCH $batch_size \
        SOLVER.ACCUMULATION_STEPS $accumulation_steps \
